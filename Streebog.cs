...
namespace GSM_Stribog
{
    public class Streebog
    {
        ...
        private static readonly byte[] IV256 = Enumerable.Repeat((byte)0x01, 64).ToArray();

        private byte[] Pi = {252, 238, 221, 17, 207, 110, 49, 22, 251, 196, 250, 218, 35, 197, 4, 77, 233, 119, 240,
        ...
        public Streebog(int hashSize, string mode)
        {
        ...
        private byte[] P_Transformation(byte[] input)
        {
            byte [] result = new byte[input.Length];
            for(int i=0;i<input.Length;i++)
            {
        ...
        private byte[] X_Transformation(byte[] a, byte[] b)
        {
        ...
        private byte[] E_Transformation(byte[] k, byte[] m)
        {
            byte[] result = (byte[])m.Clone();
            byte[] Current_Key = (byte[])k.Clone();
            for (int i = 0; i < 12; i++)
            {
                result = X_Transformation(Current_Key, result);
                result = SPL_Transformation(result);
                Current_Key = SPL_Transformation(X_Transformation(Current_Key, C[i]));
            }
            result = X_Transformation(result, Current_Key);
            return result;
        }

        private byte[] G_Transformation(byte[] N, byte[] h,byte [] m)
        {
            byte[] result = new byte[m.Length];
            byte[] key = new byte[m.Length];
            key = X_Transformation(h, N);
            key = SPL_Transformation(key);
            result = E_Transformation(key, m);
            result = X_Transformation(result, h);
            result = X_Transformation(result, m);
            return result;
        }
        ...
        public byte[] GetHash(byte[] input)
        {
            ...
            paddedMessage[64 - len - 1] = 0x01;
            h = G_Transformation(N, h, paddedMessage);
            byte[] MLen = BitConverter.GetBytes(len * 8);
            N = AddModulo512(N, MLen.Reverse().ToArray());
            Sigma = AddModulo512(Sigma, paddedMessage);
            h = G_Transformation(new byte[64], h, N);
            h = G_Transformation(new byte[64], h, Sigma);

            if(hashSize == 256)
            {
                byte[] res = new byte[32];
                Array.Copy(h, 0, res, 0, 32);
                return res;
            }
            return h;
        }
